    ...
    return new Promise<void>((resolve, reject) => {
      return this.http.get(`${environment.oauthBaseUrl}/oauth/userinfo`, { headers })
        .pipe(
          ...
          StorageService.setItem(AuthenticationService.STORAGE_KEY_USERINFO, JSON.stringify(response.record));
        });
    }).then(() => { });
  }
    ...
    return new Promise<void>((resolve, reject) => {
      return this.http.get(`${environment.oauthBaseUrl}/oauth/tokeninfo`, { headers })
        .pipe(
          ...
          StorageService.setItem(AuthenticationService.STORAGE_KEY_TOKENINFO, JSON.stringify(response));
        });
    }).then(() => { });
  }
